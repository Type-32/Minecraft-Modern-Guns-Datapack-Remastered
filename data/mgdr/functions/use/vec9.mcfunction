...
execute if entity @s[scores={fireDelay=2,vec9.fm=1}] unless score @s vec9.ammo matches ..0 run scoreboard players operation /player.lastFiredUID gamerule = @s mgdr.pid
execute if entity @s[scores={fireDelay=2,vec9.fm=1}] unless score @s vec9.ammo matches ..0 run scoreboard players operation /player.lastFiredWeaponUID gamerule = @s mgdr.weaponUID
