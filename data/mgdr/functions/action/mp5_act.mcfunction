...
execute if entity @s[predicate=mgdr:is_sneaking] anchored eyes run function mgdr:rays/mp7
execute if entity @s[predicate=!mgdr:is_sneaking] anchored eyes run function mgdr:action/recoil/spread/mp7
scoreboard players operation /player.lastFiredUID gamerule = @s mgdr.pid
scoreboard players set /player.lastFiredWeaponUID gamerule 11
tag @s remove firing
...
playsound mgdr:mp5.fire voice @a[distance=..30] ~ ~ ~ 1000
...
execute if score @s mp5.ammo matches ..0 at @s run playsound mgdr:others.empty player @s
