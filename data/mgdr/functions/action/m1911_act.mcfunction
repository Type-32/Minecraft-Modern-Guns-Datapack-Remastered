...
execute if entity @s[predicate=mgdr:is_sneaking] anchored eyes run function mgdr:rays/m1911
execute if entity @s[predicate=!mgdr:is_sneaking] anchored eyes run function mgdr:action/recoil/spread/m1911
scoreboard players operation /player.lastFiredUID gamerule = @s mgdr.pid
scoreboard players set /player.lastFiredWeaponUID gamerule 16
tag @s remove firing
