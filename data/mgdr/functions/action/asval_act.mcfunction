...
execute if entity @s[predicate=mgdr:is_sneaking] anchored eyes run function mgdr:rays/asval
execute if entity @s[predicate=!mgdr:is_sneaking] anchored eyes run function mgdr:action/recoil/spread/asval
scoreboard players operation /player.lastFiredUID gamerule = @s mgdr.pid
scoreboard players set /player.lastFiredWeaponUID gamerule 4
tag @s remove firing
