...
execute if entity @s[predicate=mgdr:is_sneaking] anchored eyes run function mgdr:rays/cz117
execute if entity @s[predicate=!mgdr:is_sneaking] anchored eyes run function mgdr:action/recoil/spread/cz117
scoreboard players operation /player.lastFiredUID gamerule = @s mgdr.pid
scoreboard players set /player.lastFiredWeaponUID gamerule 9
tag @s remove firing
