...
execute positioned ^.1 ^ ^1 if block ~ ~ ~ #mgdr:chance_penetrable if predicate mgdr:quarter_chance run function mgdr:action/recoil/spread/m14/ray_left
execute positioned ^.1 ^ ^1 run function mgdr:func/ray_reset
scoreboard players add .dist shoot_dist 1
execute if score .dist shoot_dist matches ..210 positioned ^.1 ^ ^1 if block ~ ~ ~ #mgdr:penetrable run function mgdr:action/recoil/spread/m14/ray_left
