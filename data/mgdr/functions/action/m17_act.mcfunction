...
execute if entity @s[predicate=mgdr:is_sneaking] anchored eyes run function mgdr:rays/m17
execute if entity @s[predicate=!mgdr:is_sneaking] anchored eyes run function mgdr:action/recoil/spread/m17
scoreboard players operation /player.lastFiredUID gamerule = @s mgdr.pid
scoreboard players set /player.lastFiredWeaponUID gamerule 15
tag @s remove firing
