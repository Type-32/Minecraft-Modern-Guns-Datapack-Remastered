...
data modify storage mgdr:temp data set value {Item:{Count:1b,id:"minecraft:clock",tag:{}},PickupDelay:32627,Motion:[0.0,0.0,0.0],Age:5990,Health:1}
#execute store result storage mgdr:temp data.Item.tag.CustomModelData int 1 run scoreboard players get v0 vector
execute store result storage mgdr:temp data.Motion[0] double .001 run scoreboard players get v7 vector
execute store result storage mgdr:temp data.Motion[1] double .001 run scoreboard players get v8 vector
execute store result storage mgdr:temp data.Motion[2] double .001 run scoreboard players get v9 vector
data modify entity @e[tag=bullet_shell,tag=init,limit=1,sort=nearest] {} merge from storage mgdr:temp data
