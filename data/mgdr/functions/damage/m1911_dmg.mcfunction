...
scoreboard players operation @s[type=!armor_stand] mgdr.health -= weapon.damage.m1911 mgdr.gamerule
execute if score @s mgdr.health matches 1.. run scoreboard players set @a[tag=PlayerDamageSource,scores={mgdr.weaponUID=12}] whiteHit 0
execute if score @s mgdr.health matches ..0 run scoreboard players set @a[tag=PlayerDamageSource,scores={mgdr.weaponUID=12}] redHit 0
