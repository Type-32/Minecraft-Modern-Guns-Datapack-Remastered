...
scoreboard players operation @s[type=!armor_stand] mgdr.health -= weapon.damage.vec9 mgdr.gamerule
execute if score @s mgdr.health matches 1.. run scoreboard players set @a[tag=PlayerDamageSource,scores={mgdr.weaponUID=18}] whiteHit 0
execute if score @s mgdr.health matches ..0 run scoreboard players set @a[tag=PlayerDamageSource,scores={mgdr.weaponUID=18}] redHit 0
