...
#execute as @e[distance=..0.6,type=player,gamemode=!spectator] run playsound mgdr:
##insert line of code here
execute positioned ^ ^ ^1 unless block ~ ~ ~ #mgdr:penetrable run function mgdr:func/ray_hit
execute positioned ^ ^ ^1 if block ~ ~ ~ #mgdr:chance_penetrable if predicate mgdr:tenths_chance run function mgdr:rays/ak
execute positioned ^ ^ ^1 run function mgdr:func/ray_reset
scoreboard players add .dist shoot_dist 1
execute if score .dist shoot_dist matches ..150 positioned ^ ^ ^1 if block ~ ~ ~ #mgdr:penetrable run function mgdr:rays/ak
