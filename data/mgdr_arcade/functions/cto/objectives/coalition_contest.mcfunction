...
execute if score @s objectiveCaptureDelay matches -99..99 run execute if entity @s[tag=!informCoalitionContest] run tellraw @a[team=allegiance] ["",{"text": "Objective ","color": "red"},{"score":{"name": "@s","objective": "mgdr.objectiveUID"},"color": "red"},{"text":" is being captured by Coalition!","color": "red"}]
