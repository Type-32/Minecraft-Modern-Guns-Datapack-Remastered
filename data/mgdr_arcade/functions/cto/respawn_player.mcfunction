execute if entity @s[team=allegiance] run execute as @e[tag=Allegiance,tag=CTOSpawnPoint,type=armor_stand,tag=ArcadeAsset,sort=random] at @s run function mgdr_arcade:respawn/cto/allegiance_stacktrace_spawn
