...
#execute as @a if score /player.lastFiredUID mgdr.gamerule = @s mgdr.pid run scoreboard players set @s[tag=holdingGun] che.killed_plr 1
execute as @a if score /player.lastFiredUID mgdr.gamerule = @s mgdr.pid run scoreboard players set @s che.killed_plr 1
tag @s remove tempNoFireCheck
tag @a[scores={che.killed_plr=1..},limit=1] add hasKilledPlayer
